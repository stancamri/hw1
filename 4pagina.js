...
const baseURL = '5pagina.php?id=';


function filtraRassegne(tipologia) {
    fetch('get_rassegna.php?tipologia=' + tipologia)
    .then(response => response.json())
    .then(data => {
        const rassegnaDiv = document.getElementById('rassegneVisualizzatte');
        rassegnaDiv.innerHTML = '';
        data.forEach(rassegna => {
            const rassegnaElement = document.createElement('a');
            rassegnaElement.href = baseURL + rassegna.id; // Aggiunto il link

            rassegnaElement.classList.add('avviso');
            ...
            testoAvviso.innerHTML = `<h2>${rassegna.titolo}</h2><p>${rassegna.testo}</p>`;

            contenutoAvviso.appendChild(immagine);
            contenutoAvviso.appendChild(testoAvviso);
            rassegnaElement.appendChild(contenutoAvviso);

            rassegnaDiv.appendChild(rassegnaElement);
        });
    });
}

function mostraTutteRassegne() {
    fetch('get_rassegna.php?tipologia=tutte')
    .then(response => response.json())
    .then(data => {
        const rassegnaDiv = document.getElementById('rassegneVisualizzatte');
        rassegnaDiv.innerHTML = '';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkboxes = document.querySelectorAll('input[name="tipologia"]');
