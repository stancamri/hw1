...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT titolo FROM rassegna WHERE id = $id_rassegna";
    ...
    if ($result->num_rows > 0) {
        ...
        $rassegna_titolo = $row["titolo"];
    }
}
// Ottieni le notizie dal database
$query = "SELECT r.id, r.titolo, r.testo, COUNT(l.id) as like_count 
          FROM rassegna  r
           JOIN likes l ON r.id = l.rassegna_id 
          GROUP BY r.id";
$result = $mysqli->query($query);
...
<head>
    ...
    <title><?php echo $rassegna_titolo; ?></title>
    <link rel="stylesheet" href="5pagina.css">
    <script src="5pagina.js" defer></script>
...
<div id="flex-container3">
  ...
  <div class="container">
        ...
        if (isset($_GET['id'])) {
            ...
            $query = "SELECT r.*, 
            (SELECT COUNT(*) FROM likes WHERE rassegna_id = r.id) AS like_count 
     FROM rassegna r 
     WHERE r.id = $id_rassegna";
...
$result = $mysqli->query($query);
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                  $liked = $row['user_like'] > 0;
                    ...
                    echo "</div>";
                    echo "<div id='bott'>";
                    $query = "SELECT * FROM likes WHERE rassegna_id = ? AND user_id = ?";
                    ...
                    echo "<button class='like-btn' data-id='" . $row['id'] . "' " . ($liked ? "disabled" : "") . ">❤️ <span class='like-count'>" . $row['like_count'] . "</span></button>";
                  echo "</div>";
                  echo "</div>";
        ...
        ?>
        <!-- Modale per mostrare chi ha messo mi piace -->
