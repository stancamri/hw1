...
if (isset($_GET['query'])) {
    ...
    // Configurazione della richiesta cURL
    $curl = curl_init();
    ...
    if (isset($booksData['docs']) && count($booksData['docs']) > 0) {
        echo "<h2>Risultati della ricerca per: $searchTerm</h2>";
        echo "<div class='container'>";
        foreach ($booksData['docs'] as $book) {
            echo "<div class='book'>";
            echo "<h3>" . $book['title'] . "</h3>";
            if (isset($book['author_name'])) {
                echo "<p>Autore: " . implode(", ", $book['author_name']) . "</p>";
            } else {
                echo "<p>Autore: Non disponibile</p>";
            }
            if (isset($book['cover_i'])) {
                echo "<img src='http://covers.openlibrary.org/b/id/{$book['cover_i']}-L.jpg' alt='Immagine libro'>";
            } else {
                echo "<p>Immagine libro non disponibile</p>";
            }
            if (isset($book['ia'])) {
                echo "<p><a href='https://archive.org/details/{$book['ia'][0]}' target='_blank'>Visualizza Anteprima PDF</a></p>";
            } else {
                echo "<p>Anteprima PDF non disponibile</p>";
            }
            echo "</div>";
        }
        echo "</div>";
    } else {
        echo "<h2>Nessun libro trovato per: $searchTerm</h2>";
    }
} else {
